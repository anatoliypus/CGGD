...
namespace cg
{
	...
	struct unsigned_color
	{
		...
		{
			unsigned_color c{};
			c.r = std::clamp(static_cast<int>(255.f * color.r), 0, 255);
			c.g = std::clamp(static_cast<int>(255.f * color.g), 0, 255);
			c.b = std::clamp(static_cast<int>(255.f * color.b), 0, 255);
			return c;
		};
		static unsigned_color from_float3(const float3& color)
		{
			...
			c.r = static_cast<uint8_t>(preprocessed.x);
			c.g = static_cast<uint8_t>(preprocessed.y);
			c.b = static_cast<uint8_t>(preprocessed.z);
			return c;
		};
		float3 to_float3() const
		{
